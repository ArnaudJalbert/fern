...
def test_open_vault_success() -> None:
    ...
    db = Database(name="Inbox", pages=[page], properties=[prop, title_prop], property_order=["id", "title"])
...
def test_open_vault_includes_schema_and_property_order() -> None:
    ...
    db = Database(
        name="D",
        pages=[page],
        properties=[id_prop, title_prop, Property(id="p1", name="Status", type=PropertyType.STRING)],
...
def test_open_vault_default_order_when_empty() -> None:
    id_prop = Property(id="id", name="ID", type=PropertyType.ID, mandatory=True)
    title_prop = Property(id="title", name="Title", type=PropertyType.TITLE, mandatory=True)
    db = Database(name="D", pages=[], properties=[id_prop, title_prop], property_order=[])
    vault = Vault(name="V", databases=[db])
    repo = MagicMock()
    repo.get.return_value = vault
