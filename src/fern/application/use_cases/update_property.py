...
class UpdatePropertyUseCase:
    """Update a property's name and/or type in the manifest and on every page."""
    ...
    def execute(self, input_data: Input) -> Output:
        ...
        idx = next(
            (i for i, p in enumerate(manifest.properties) if p.id == input_data.property_id),
            None,
        )
        if idx is None:
            return self.Output(success=False)

        old = manifest.properties[idx]
        ...
        new_props = [*manifest.properties]
        new_props[idx] = updated
        self._manifest_repository.save(Manifest(properties=new_props))
